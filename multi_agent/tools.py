...
async def fetch_weather(city: str) -> Dict[str, Any]:
    ...
    Returns a dict of weather fields on success, or a dict with an ``"error"`` key
    on failure (missing API key, unknown city, auth error, timeout, etc.).
    """
    ...
    try:
        ...
        return {
            "error": f"HTTP {ex.response.status_code} error while fetching weather for city: '{city}'"
        }
